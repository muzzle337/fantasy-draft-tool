...
const BRAIN_URL = "./draft-brain-v6-5.json?v=6.6.7";
...
async function loadBrain() {
  ...
  leagueStrategy = data.draft_logic || data.league_strategy || {};

  dstStreaming = new Map(
    (data.dst_streaming || []).map((item) => [item.abbr, {
      rank:item.rank ?? item.stream_rank,
      opponent:item.week1_opponent,
      grade:item.grade ?? item.week1_grade,
      strategy:item.strategy,
      note:item.note
    }])
  );

  const kickerPriority = (data.kicker_strategy && Array.isArray(data.kicker_strategy.priority))
    ? data.kicker_strategy.priority
    : [];
  kickerStreaming = new Map(
    kickerPriority.map((name,index) => [name, {
      rank:index + 1,
      opponent:null,
      strategy:"FINAL_OR_NEAR_FINAL",
      note:data.kicker_strategy?.rule || "Draft kicker only late."
    }])
  );
...
function runSelfCheck() {
  const requiredIds = [
    ...
    "recentPicksToggle","recentPicksList","debugModal","recommendationEyebrow","recommendationTitle","bestAvailableCard","toolsView","starterRoster","benchRoster","irRoster","saveHealthButton","createBackupButton","restoreBackupButton","exportStateButton","importStateInput","resetCurrentDraftButton","clearAllLocalDataButton","backupModal","restoreModal"
  ];
...
function picksUntilMyTurn(state = currentState()) {
  const nextPick = nextMyPick(state);
  if (nextPick === null) return null;
  return Math.max(0, nextPick - currentOverallPick(state));
}

function isMyTurn(state = currentState()) {
  return picksUntilMyTurn(state) === 0;
}

function marketExpectedPick(player) {
  // Yahoo is the real market signal when present.
  // our_rank only acts as a simulator/ordering anchor when Yahoo is unavailable.
  return yahooAdpAvailable(player) ? Number(player.yahooAdp) : Number(player.overallRank);
}

function likelyNextScore(player, state = currentState()) {
  const pick = currentOverallPick(state);
  const expected = marketExpectedPick(player);
  const overdue = Math.max(0, pick - expected);
  const upcoming = Math.max(0, expected - pick);

  let score = 260 - upcoming * 4 + overdue * 12;
  if (yahooAdpAvailable(player)) score += 18;
  if (player.overallRank <= pick + 8) score += 8;
  if (player.position === "K" || player.position === "DEF") score -= currentRound(state) < 12 ? 120 : 0;
  return score;
}
...
function recommendationLabel(player, state = currentState()) {
  ...
  if (player.doNotReachStart && round < player.doNotReachStart) return "DON'T REACH";
  if (remaining === 1 && ["RB","WR","TE","QB"].includes(player.position)) return "LAST OF TIER";
  if (player.injurySeverity >= 3 || player.stance === "INJURY_DISCOUNT_ONLY") return "WAIT";
  if (player.targetRoundStart && round < player.targetRoundStart) return "WAIT";
  if (["STRONG_TARGET","STRONG_TARGET_AT_DISCOUNT"].includes(player.stance) &&
      (!player.targetRoundStart || round >= player.targetRoundStart)) return "TAKE NOW";
  return "GOOD PICK";
}
...
function injuryDisplay(player) {
  const status = safeText(player.injuryStatus,"No issue tracked").replaceAll("_"," ");
  const availability = safeText(player.week1Availability,"").replaceAll("_"," ");

  if (player.injurySeverity >= 3) {
    return { text:`${status}${availability ? ` · ${availability}` : ""}`, level:"high" };
  }
  if (player.injurySeverity >= 1 || /UNCERTAIN|POSSIBLE|VERIFY|LIMITED|MONITOR|PUP/i.test(`${status} ${availability}`)) {
    return { text:`${status}${availability ? ` · ${availability}` : ""}`, level:"medium" };
  }
  return { text:"Healthy / no major issue tracked", level:"low" };
}

function compactThesis(text, max=220) {
  const value = safeText(text,"");
  if (value.length <= max) return value;
  return `${value.slice(0,max).trim()}…`;
}

function importantTags(player, state = currentState()) {
  const tags = [];
  const source = new Set(player.tags || []);
  if (source.has("BOONE_LEAGUE_WINNER")) tags.push("BOONE LEAGUE WINNER");
  if (source.has("STRONG_TARGET") || player.stance === "STRONG_TARGET") tags.push("STRONG TARGET");
  if (source.has("VALUE")) tags.push("VALUE");
  if (player.stance === "DO_NOT_REACH" || source.has("DO_NOT_REACH")) tags.push("DO NOT REACH");
  if (player.injurySeverity >= 2 || injuryTag(player)) tags.push("INJURY");
  if (tierRemaining(player,state) === 1 && ["RB","WR","TE","QB"].includes(player.position)) tags.push("LAST OF TIER");
...
function playerButtonHtml(player, state = currentState(), {confirmMyPick=true, showMyPick=true} = {}) {
  const drafted = isDrafted(player.id, state);
  const mine = state.myPlayers.includes(player.id);
  const injury = injuryDisplay(player);
  const thesis = safeText(player.thesis,"No custom thesis yet.");
  const tags = importantTags(player,state);

  return `
    <article class="player-card ${drafted ? "drafted-player" : ""} ${player.stance === "STRONG_TARGET" ? "strong-target-card" : ""}">
      <button type="button" class="player-details-button" data-open-player="${player.id}">
        ...
        <div class="injury-line injury-${injury.level}">
          <strong>Injury:</strong> ${injury.text}
        </div>
        <p class="player-thesis"><strong>Our Thesis:</strong> ${thesis}</p>
        ${tags.length ? `<div class="player-tags">${tags.map(tag => `<span class="tag ${tagClass(tag)}">${tag}</span>`).join("")}</div>` : ""}
      </button>
      ${drafted ? "" : `
        <div class="player-quick-actions">
          <button type="button" class="quick-drafted-button" data-draft-other="${player.id}">Drafted</button>
          ${showMyPick ? `<button type="button" class="quick-my-pick-button" ${confirmMyPick ? `data-my-pick-confirm="${player.id}"` : `data-my-pick-direct="${player.id}"`}>My Pick</button>` : ""}
        </div>
      `}
    </article>
  `;
...
function renderSearchResults() {
  ...
  el("searchResults").innerHTML = matches.length ? matches.map((p) => playerButtonHtml(p,state,{confirmMyPick:true,showMyPick:true})).join("") : '<p class="muted">No players found.</p>';
...
function renderPlayerBoard() {
  ...
  el("playerList").innerHTML = board.length ? board.map((p) => playerButtonHtml(p,state,{confirmMyPick:true,showMyPick:true})).join("") : '<p class="muted">No players found.</p>';
}

function renderBestAvailable() {
  const state = currentState();
  const card = el("bestAvailableCard");
  ...
  el("bestAvailableList").innerHTML = list.length
    ? list.map((p) => playerButtonHtml(p,state,{confirmMyPick:false,showMyPick:true})).join("")
    : '<p class="muted">No players remaining.</p>';
...
function renderRecommendations() {
  const state = currentState();
  const waiting = !isMyTurn(state);

  el("recommendationEyebrow").textContent = waiting ? "While You Wait" : "Your Decision";
  el("recommendationTitle").textContent = waiting ? "Likely Next" : "Recommended for You";
  el("showFocusButton").classList.toggle("hidden", waiting);

  if (waiting) {
    const away = picksUntilMyTurn(state);
    const likely = availablePlayers(state)
      .map((p) => ({...p, likelyScore:likelyNextScore(p,state)}))
      .sort((a,b) => b.likelyScore - a.likelyScore || marketExpectedPick(a) - marketExpectedPick(b))
      .slice(0,5);

    el("recommendationContext").textContent = away === null ? "Set draft slot" : `${away} picks until you`;
    el("recommendationPanel").innerHTML = likely.length ? likely.map((p,index) => {
      const injury = injuryDisplay(p);
      const market = yahooAdpAvailable(p) ? `Yahoo ${p.yahooAdp}` : `Our #${p.overallRank}`;
      return `
        <article class="likely-next-row ${index===0 ? "most-likely" : ""}">
          <button type="button" class="likely-next-details" data-open-player="${p.id}">
            <div class="recommendation-rank">${index+1}</div>
            <div class="player-main">
              <div class="player-name">${p.name} — ${p.position}</div>
              <div class="player-meta">${market} · ${p.tierLabel}</div>
              <div class="injury-line injury-${injury.level}"><strong>Injury:</strong> ${injury.text}</div>
              <p class="recommendation-thesis"><strong>Our Thesis:</strong> ${compactThesis(p.thesis)}</p>
              <div class="player-tags">${importantTags(p,state).map(tag => `<span class="tag ${tagClass(tag)}">${tag}</span>`).join("")}</div>
            </div>
          </button>
          <button type="button" class="likely-drafted-button" data-draft-other="${p.id}">Drafted</button>
        </article>
      `;
    }).join("") : '<p class="muted">No likely-next players available.</p>';
    return;
  }
  ...
  el("recommendationPanel").innerHTML = recs.length ? recs.map((p,index) => {
    ...
    const injury = injuryDisplay(p);
    return `
      <article class="recommendation-row ${index===0 ? "top-pick" : ""}">
        <button type="button" class="recommendation-details" data-open-player="${p.id}">
          <div class="recommendation-rank">${index+1}</div>
          <div class="player-main">
            <div class="player-name">${p.name} — ${p.position}</div>
            <div class="player-meta">Our #${p.overallRank} · ${p.tierLabel} · Role ${p.roleConfidence}/10</div>
            <div class="player-tags"><span class="tag ${labelClass}">${label}</span>${importantTags(p,state).map(tag => `<span class="tag ${tagClass(tag)}">${tag}</span>`).join("")}</div>
            <div class="injury-line injury-${injury.level}"><strong>Injury:</strong> ${injury.text}</div>
        ...
        <button type="button" class="recommended-my-pick-button" data-my-pick-direct="${p.id}">My Pick</button>
      </article>
    `;
...
function renderCliffs() {
  ...
  el("cliffPanel").innerHTML = `
    <p class="cliff-title">⚠ ${warning.sameTier.length === 1 ? "LAST IN TIER" : `${warning.position} CLIFF`}</p>
...
function rosterNeedLabel(position,state = currentState()) {
  const round = currentRound(state);
  const need = skillStarterNeed(position,state);
  const counts = rosterCounts(state);

  if (position === "RB") {
    if (need > 0) return {label:"HIGH",klass:"high"};
    return {label:(counts.RB || 0) >= 3 ? "OK" : "MED",klass:(counts.RB || 0) >= 3 ? "ok" : "med"};
  }
  if (position === "WR") {
    if (need > 0) return {label:"HIGH",klass:"high"};
    return {label:"OK",klass:"ok"};
  }
  if (position === "QB") {
    if (!need) return {label:"SET",klass:"ok"};
    return round < 5 ? {label:"WAIT",klass:"wait"} : {label:"NEED",klass:"high"};
  }
  if (position === "TE") {
    if (!need) return {label:"SET",klass:"ok"};
    return round < 5 ? {label:"WAIT",klass:"wait"} : {label:"NEED",klass:"med"};
  }
  return {label:"—",klass:"ok"};
}

function renderRosterNeeds() {
  const state = currentState();
  const positions = ["RB","WR","QB","TE"];
  el("rosterNeedsBar").innerHTML = positions.map((position) => {
    const need = rosterNeedLabel(position,state);
    return `<span class="need-pill ${need.klass}"><strong>${position}</strong> ${need.label}</span>`;
  }).join("");

  const nextPick = nextMyPick(state);
  const away = nextPick === null ? null : Math.max(0,nextPick-currentOverallPick(state));
  const urgency = el("turnUrgency");
  if (!state.draftSlot) {
    urgency.textContent = "Set draft slot";
    urgency.className = "turn-urgency";
  } else if (away === 0) {
    urgency.textContent = "YOU'RE ON THE CLOCK";
    urgency.className = "turn-urgency urgent";
  } else if (away <= 3) {
    urgency.textContent = `${away} pick${away===1?"":"s"} away`;
    urgency.className = "turn-urgency near";
  } else {
    urgency.textContent = `${away} picks away`;
    urgency.className = "turn-urgency";
  }
}

function bestAvailableByPosition(position,state = currentState()) {
  return availablePlayers(state)
    .filter(p => p.position === position)
    .sort((a,b) => recommendationScore(b,state)-recommendationScore(a,state) || a.overallRank-b.overallRank)[0] || null;
}

function focusShouldAutoOpen(state = currentState()) {
  if (!state.draftSlot || !isMyTurn(state)) return false;
  const nextPick = nextMyPick(state);
  if (nextPick === null) return false;
  return Math.max(0,nextPick-currentOverallPick(state)) <= 3;
}

let focusManuallyShown = false;
let focusManuallyHiddenAtPick = null;
...
function renderFocusMode() {
  ...
  const shouldAuto = focusShouldAutoOpen(state) && focusManuallyHiddenAtPick !== currentPick;
  if (!focusManuallyShown && !shouldAuto) {
    card.classList.add("hidden");
    return;
  }
  ...
  const bestByPos = ["RB","WR","QB","TE"].map(pos => bestAvailableByPosition(pos,state)).filter(Boolean);
  const needs = ["RB","WR","QB","TE"].map(pos => {
    const x = rosterNeedLabel(pos,state);
    return `${pos} ${x.label}`;
  }).join(" · ");

  el("focusPanel").innerHTML = `
    <div class="focus-needs">${needs}</div>
    <div class="focus-list">
      ${recs.map((p,index) => {
        const injury = injuryDisplay(p);
        return `
          <button class="focus-player" type="button" data-open-player="${p.id}">
            <span class="focus-rank">${index+1}</span>
            <span class="focus-copy">
              <strong>${p.name} · ${p.position}</strong>
              <small>${recommendationLabel(p,state)} · ${injury.level === "high" ? "⚠ Injury risk" : injury.level === "medium" ? "Injury watch" : "Healthy"}</small>
              <em>${safeText(p.thesis).slice(0,150)}</em>
            </span>
          </button>`;
      }).join("")}
    </div>
    <div class="focus-best">
      ${bestByPos.map(p => `<button type="button" data-open-player="${p.id}"><span>${p.position}</span><strong>${p.name}</strong></button>`).join("")}
    </div>
  `;
  card.classList.remove("hidden");
}
...
function handleDelegatedClick(event) {
  const draftOther = event.target.closest("[data-draft-other]");
  if (draftOther) {
    event.preventDefault();
    event.stopPropagation();
    return recordAction("draftedByOther",draftOther.dataset.draftOther);
  }

  const directMyPick = event.target.closest("[data-my-pick-direct]");
  if (directMyPick) {
    event.preventDefault();
    event.stopPropagation();
    return recordAction("myPick",directMyPick.dataset.myPickDirect);
  }

  const confirmedMyPick = event.target.closest("[data-my-pick-confirm]");
  if (confirmedMyPick) {
    event.preventDefault();
    event.stopPropagation();
    const player = getPlayer(confirmedMyPick.dataset.myPickConfirm);
    if (player && window.confirm(`Add ${player.name} to My Team?`)) {
      return recordAction("myPick",player.id);
    }
    return;
  }
...
function bindStaticEvents() {
  ...
  el("showFocusButton").addEventListener("click",() => {
    focusManuallyShown = true;
    focusManuallyHiddenAtPick = null;
    renderFocusMode();
    el("focusCard").scrollIntoView({behavior:"smooth",block:"start"});
  });

  el("hideFocusButton").addEventListener("click",() => {
    focusManuallyShown = false;
    focusManuallyHiddenAtPick = currentOverallPick(currentState());
    el("focusCard").classList.add("hidden");
  });
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    navigator.serviceWorker
      .register("./service-worker-v6-5.js", { scope: "./" })
      .then((registration) => {
...
function applyCompactView(){const on=compactViewEnabled();document.body.classList.toggle('compact-view',on);}
...
const modeToggleTarget = el('modeBanner');
if (modeToggleTarget) {
