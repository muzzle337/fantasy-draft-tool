const BUILD_VERSION = "6.4.0-draft-flow";
const TEAM_COUNT = 14;
...
const BRAIN_URL = "./draft-brain-v6-4.json?v=6.4.0";
...
function runSelfCheck() {
  const requiredIds = [
    ...
    "recentPicksToggle","recentPicksList","debugModal","recommendationEyebrow","recommendationTitle","bestAvailableCard","draftQuickBar","stickyUndoButton","stickyToolsButton","quickToolsModal","starterRoster","benchRoster","irRoster","saveHealthButton","createBackupButton","restoreBackupButton","exportStateButton","importStateInput","resetCurrentDraftButton","clearAllLocalDataButton","backupModal","restoreModal"
  ];
        ...
        <article class="likely-next-row ${index===0 ? "most-likely" : ""}">
          <button type="button" class="likely-next-details" data-open-player="${p.id}">
            ...
            <div class="player-main">
              ...
              <p class="recommendation-thesis"><strong>Our Thesis:</strong> ${safeText(p.thesis)}</p>
...
function openQuickToolsModal() {
  el("quickToolsModal").classList.remove("hidden");
}

function closeQuickToolsModal() {
  el("quickToolsModal").classList.add("hidden");
}
...
function handleDelegatedClick(event) {
  ...
  if (quickTool) {
    closeQuickToolsModal();
    const action = quickTool.dataset.quickTool;
    if (action === "backup") return createManualBackup();
    if (action === "restore") return openRestoreModal();
    if (action === "export") return exportState();
    if (action === "reset") return openResetModal("current");
  }

  if (event.target.closest("[data-close-quick-tools]")) return closeQuickToolsModal();
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    navigator.serviceWorker
      .register("./service-worker-v6-4.js", { scope: "./" })
      .then((registration) => {
