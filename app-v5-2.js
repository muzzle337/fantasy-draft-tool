const BUILD_VERSION = "5.2.0-offline";
const TEAM_COUNT = 14;
const STORAGE_PREFIX = "fantasyDraftToolStateV5";
const BRAIN_URL = "./draft-brain-v5-2.json?v=5.2.0";
...
function activeMode() {
  return sessionStorage.getItem("fantasyDraftActiveModeV5") || "live";
}

function setActiveMode(mode) {
  sessionStorage.setItem("fantasyDraftActiveModeV5", mode);
}
...
function runSelfCheck() {
  const requiredIds = [
    ...
    "recentPicksToggle","recentPicksList","debugModal","starterRoster","benchRoster","irRoster"
  ];
...
function adpConfidenceWeight(quality) {
  ...
  if (q === "QUOTED") return 1;
  if (q === "ESTIMATE") return .55;
  if (q === "PROXY") return .20;
  return .15;
}
...
function priceDisciplineAdjustment(player, round, state = currentState()) {
  ...
  if (Number.isFinite(Number(player.marketAdp))) {
    ...
    const gap = currentOverallPick(state) - Number(player.marketAdp);
...
function recommendationReason(player, state = currentState()) {
  ...
  if (!reasons.length) reasons.push(player.thesis || "Strong combination of rank, tier, roster fit and price.");
...
function openPlayerModal(playerId) {
  ...
  el("modalPlayerContent").innerHTML = `
    ...
    <div class="detail-grid">
      ...
      <div class="detail-box"><span>Market ADP</span><strong>${safeText(player.marketAdp)} (${safeText(player.marketAdpQuality)})</strong></div>
...
function realisticMockCandidate(state) {
  ...
  const ranked = available.map((p) => {
    const adp = Number.isFinite(Number(p.marketAdp)) ? Number(p.marketAdp) : p.overallRank;
    const marketWeight = adpConfidenceWeight(p.marketAdpQuality);
    const blended = p.overallRank * (1 - .45 * marketWeight) + adp * (.45 * marketWeight);
    const distance = Math.abs(blended - currentOverallPick(state));
    const rankPenalty = Math.max(0,p.overallRank-currentOverallPick(state))*0.08;
    return {player:p,base:distance+rankPenalty};
  }).sort((a,b) => a.base-b.base);

  const window = ranked.slice(0,Math.min(10,ranked.length));
  const weighted = window.map((item,index) => ({
    ...item,
    weight: Math.max(.1, 1.4 - index*.12) * (0.8 + Math.random()*.4)
  }));
...
function bindStaticEvents() {
  ...
  el("fullResetButton").addEventListener("click",openResetModal);
...
if ("serviceWorker" in navigator) {
  window.addEventListener("load", () => {
    navigator.serviceWorker
      .register("./service-worker-v5-2.js", { scope: "./" })
      .then((registration) => {
